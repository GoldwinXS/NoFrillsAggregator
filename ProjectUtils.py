...
def prepare_csv(data):
    ...
    get_df = lambda name: data_df[data_df['person'] == name]['data'].values[0] # 0th index because I'm expecting only 1 value
    ...
    csv_data = csv_data.append(pd.Series(0, index=csv_data.columns), ignore_index=True) # add a row for taxes for each person
